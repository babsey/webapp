...
def voltmeter(request, result_id):
	...
	'name': '%s [%s]' %(readable(status[1]['label']), status[0]),
	}
...
def voltmeter_thumbnail(request, result_id):
    ...
    result_obj = get_object_or_404(Result, pk=result_id)
    voltmeter = result_obj.voltmeter_data()
    data = [{'values':d['values'], 'name':'%s [%s]' %(readable(d['status'][1]['label']), d['status'][0])} for d in voltmeter['V_m']]
    response = {'data': data, 'times': voltmeter['times']}
    return {'voltmeter': cjson.encode(response)}
...
def spike_detector(request, result_id):
    ...
    spike_detector['neurons'] = [nn[0] for nn in result_obj.network.device_list('neuron')]
